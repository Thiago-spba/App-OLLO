...
const VerifyEmailPage = () => {
  ...
  useEffect(() => {
    // Se, por algum motivo, o usuário não estiver logado ou já estiver verificado,
    // redireciona para a home para evitar que esta página seja acessada indevidamente.
  ...
  const handleResendEmail = async () => {
    if (!auth.currentUser) {
      toast.error('Você precisa estar logado para reenviar o e-mail.');
      navigate('/login');
      return;
    }
    try {
  ...
  if (!user) {
    // Renderiza um loader ou nada enquanto o estado do usuário está sendo carregado
    return (
      <div className="flex items-center justify-center min-h-screen bg-ollo-light-bg dark:bg-ollo-dark-bg">
        <p className="text-ollo-dark-text dark:text-ollo-light-text">
          Carregando...
        </p>
      </div>
    );
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="max-w-md w-full p-8 space-y-6 bg-white dark:bg-gray-800 rounded-lg shadow-md text-center">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">
          Após verificar, você pode fechar esta aba e fazer o login.
        </p>
